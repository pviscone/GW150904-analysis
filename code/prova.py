...
#dt_crop=((gw.data.H1.times.value[-1]-gw.data.H1.times.value[0])/2)-30
dt_crop=2
...
# %%
plt.figure()
plt.imshow(H1_max_snr,origin="lower",aspect="auto",extent=(mx[0],mx[-1],my[0],my[-1]))
plt.colorbar()
...
# %%
m1_idx,m2_idx=np.unravel_index(H1_max_snr.argmax(), H1_max_snr.shape)
plt.figure()
plt.plot(H1_cbc.crop(4,4).sample_times-gps,np.abs(H1_snr[m2_idx,m1_idx]))

plt.title(f"Best SNR match for H1:   $m_1$:{m[m1_idx]:.1f} $ \pm $ {(m[-1]-m[0])/n:.1f} $m_\odot$, $m_2$={m[m2_idx]:.1f} $\pm$ {(m[-1]-m[0])/n:.1f} $m_\odot$")
...
# %%
plt.figure()
plt.imshow(L1_max_snr,origin="lower",aspect="auto",extent=(mx[0],mx[-1],my[0],my[-1]))
plt.colorbar()
...
# %%
m1_L1_idx,m2_L1_idx=np.unravel_index(L1_max_snr.argmax(), L1_max_snr.shape)
plt.figure()
plt.plot(H1_cbc.crop(4,4).sample_times-gps,np.abs(L1_snr[m2_L1_idx,m1_L1_idx]))

plt.title(f"Best SNR match for H1:   $m_1$:{m[m1_L1_idx]:.1f} $ \pm $ {(m[-1]-m[0])/n:.1f} $m_\odot$, $m_2$={m[m2_L1_idx]:.1f} $\pm$ {(m[-1]-m[0])/n:.1f} $m_\odot$")
plt.xlabel("Time (after ev_gps) [s]")
plt.ylabel("SNR")
plt.show()
